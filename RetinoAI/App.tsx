...
// MANDATORY: ONNX Environment for AWS Deployment
...
// MANDATORY: Configure WASM paths for Amplify root directory
...
const App: React.FC = () => {
  ...
  const processFile = async (file: File) => {
    if (modelStatus !== 'ready') {
      setError(lang === 'hi' ? "मॉडल लोड हो रहा है..." : "Model not initialized.");
      return;
    }
    setError(null);
    setIsAnalyzing(true);
    try {
      ...
      try {
        parsedExplanation = typeof rawExplanation === 'string' ? JSON.parse(rawExplanation) : rawExplanation;
      } catch (e) {
        console.warn("JSON Parse failed, using raw response");
        parsedExplanation = rawExplanation;
      }
          ...
          <div className="animate-fadeIn space-y-12">
            <div className="bg-white p-10 md:p-14 rounded-[3.5rem] shadow-sm border border-slate-100 flex flex-col md:flex-row items-center gap-12 group relative overflow-hidden">
               ...
               <div className="flex-grow space-y-8 z-10 text-center md:text-left">
                  ...
                  <label className={`inline-flex px-10 py-5 rounded-[2.5rem] font-black uppercase tracking-widest text-xs shadow-2xl transition-all gap-4 items-center 
                    ${(modelStatus === 'ready' && !isAnalyzing) 
                      ? 'bg-blue-600 text-white cursor-pointer hover:bg-blue-700 shadow-blue-600/30' 
                      : 'bg-slate-400 text-white cursor-not-allowed opacity-70 shadow-none'}`}>
                    ...
                    <span>
                        {modelStatus === 'loading' ? "Initializing AI..." : isAnalyzing ? analysisStep : t.upload}
                    </span>
                       ...
                       <div key={record.id} className="bg-white p-8 md:p-10 rounded-[3rem] shadow-sm border border-slate-100 flex flex-col md:flex-row gap-10">
                         ...
                         <div className="flex-grow space-y-6">
                            ...
                            <div className="flex gap-3 pt-2">
                              <button onClick={() => handleSpeak(record)} className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-black uppercase text-[10px] border border-slate-200 transition-all ${speakingId === record.id ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-blue-50'}`}>
                  ...
                  <div key={record.id} className="bg-white p-10 rounded-[3.5rem] shadow-sm border border-slate-100 grid grid-cols-1 md:grid-cols-12 gap-12 relative overflow-hidden">
                    ...
                    <div className="md:col-span-5 space-y-8">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-5 gap-3">
                          {[0, 1, 2, 3, 4].map(g => (
                            <div key={g} className={`py-4 rounded-2xl text-[11px] font-black border text-center transition-all ${record.aiGrade === g ? 'bg-slate-900 text-white shadow-xl scale-105 border-slate-900' : 'bg-slate-50 text-slate-300 border-slate-100 opacity-50'}`}>{g}</div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <textarea className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-xs font-medium focus:ring-2 focus:ring-blue-500 outline-none transition-all" placeholder="Notes..." rows={3} value={feedbackText[record.id] || ""} onChange={(e) => setFeedbackText({...feedbackText, [record.id]: e.target.value})} />
                      </div>
                    </div>
                    ...
                    <div className="md:col-span-3 flex flex-col justify-end gap-3 border-l border-slate-100 pl-8">
                      <button 
                        ...
                        className={`w-full py-5 rounded-2xl font-black uppercase text-[11px] transition-all 
                          ${record.status === 'APPROVED' ? 'bg-green-600 text-white shadow-none opacity-100' : 
                            record.status === 'REJECTED' ? 'bg-slate-200 text-slate-400 cursor-not-allowed opacity-50' : 
                            'bg-blue-600 text-white shadow-lg shadow-blue-500/30 hover:bg-blue-700'}`}
                      >
                        {record.status === 'APPROVED' ? 'Verified & Signed' : 'Confirm & Sign'}
                      </button>

                      <button 
                        onClick={() => handleRejectRecord(record.id)} 
                        disabled={record.status !== 'PENDING'}
                        className={`w-full py-5 rounded-2xl font-black uppercase text-[11px] transition-all border-2 
                          ${record.status === 'REJECTED' ? 'bg-red-600 text-white border-red-600 opacity-100' : 
                            record.status === 'APPROVED' ? 'bg-slate-100 text-slate-300 border-slate-100 cursor-not-allowed opacity-50' : 
                            'bg-white text-red-600 border-red-100 hover:bg-red-50'}`}
                      >
                        {record.status === 'REJECTED' ? 'Diagnostic Rejected' : 'Reject Diagnostic'}
                      </button>
                    </div>
                  </div>
                ))}
